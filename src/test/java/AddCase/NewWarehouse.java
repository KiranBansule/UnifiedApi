...
public class NewWarehouse {
	...
	@Test(dataProvider = "NewWarehouse")
	void PatchNewWarehouse(String caseId, String warehouseName, String warehouseCode, String warehousePhoneNo,
			String warehousePin, String expectedStatus, String expectedMessage,String Output) {

		PropertiesUrl proper = new PropertiesUrl();
		String url = proper.getbasedUrl();
		String id = caseId;
		String requestBody = "{\"warehouseName\": \"%s\",\"warehouseCode\": \"%s\",\"warehousePhoneNo\": \"%s\",\"warehousePin\": \"%s\"}"
				.formatted(warehouseName, warehouseCode, warehousePhoneNo, warehousePin);
		...
		io.restassured.response.Response response = given().header("Authorization", "Bearer " + jwtToken)
				...
				.when().patch(url + "/cases/{caseId}/new-warehouse").then()
	...
	@DataProvider(name = "NewWarehouse")
	Object[][] getCaseData() throws IOException {
		...
		int rownum = XLUtils.getRowCount(path, "NewWarehouse");
		int colnum = XLUtils.getCellCount(path, "NewWarehouse", 1);

		Object warehouseData[][] = new Object[rownum][colnum];

		for (int i = 1; i <= rownum; i++) {
			for (int j = 0; j < colnum; j++) {
				warehouseData[i - 1][j] = XLUtils.getCellData(path, "NewWarehouse", i, j);
			}
		}
