...
public class SaveBank {
	...
	void PatchSaveBank(String caseId, String userBankAccountId,String expectedStatus,String expectedMessage,String Output) {

		PropertiesUrl proper = new PropertiesUrl();
		String url = proper.getbasedUrl();
		String id = caseId;
		String requestBody = "{\"userBankAccountId\": \"%s\"}".formatted(userBankAccountId);
		...
		io.restassured.response.Response response = given().header("Authorization", "Bearer " + jwtToken)
				...
				.when().patch(url + "/cases/{caseId}/bank").then()
	...
	Object[][] getCaseData() throws IOException {
		...
		int rownum = XLUtils.getRowCount(path, "BankAccount");
		int colnum = XLUtils.getCellCount(path, "BankAccount", 1);

		Object bankData[][] = new Object[rownum][colnum];

		for (int i = 1; i <= rownum; i++) {
			for (int j = 0; j < colnum; j++) {
				bankData[i - 1][j] = XLUtils.getCellData(path, "BankAccount", i, j);
			}
		}
		return bankData;
	}
