...
public class SaveSchedule {
	...
	void PatchSaveSchedule(String caseId, String dates, String pomId,String expectedStatus,String expectedMessage,String Output) {

		PropertiesUrl proper = new PropertiesUrl();
		String url = proper.getbasedUrl();
		String id = caseId;
		String requestBody = "{\"dates\": %s,\"pomId\": %s}".formatted(dates, pomId);
		...
		io.restassured.response.Response response = given().header("Authorization", "Bearer " + jwtToken)
				...
				.when().patch(url + "/cases/{caseId}/schedule").then()
	...
	Object[][] getCaseData() throws IOException {
		...
		int rownum = XLUtils.getRowCount(path, "Schedule");
		int colnum = XLUtils.getCellCount(path, "Schedule", 1);

		Object scheduleData[][] = new Object[rownum][colnum];

		for (int i = 1; i <= rownum; i++) {
			for (int j = 0; j < colnum; j++) {
				scheduleData[i - 1][j] = XLUtils.getCellData(path, "Schedule", i, j);
			}
		}
		return scheduleData;
	}
