...
public class CaseDetails {
	...
	void getCaseDetails(String caseId) {
		...
		given().header("Authorization", "Bearer " + jwtToken)
				...
				.when().get(url + "/cases/{caseId}/details").then()//
				// .assertThat()
				.body(JsonSchemaValidator.matchesJsonSchemaInClasspath("MasterJsonSchema\\case_details.json")).log()
				.all();
	}
