...
public class POM {
	...
	void getPom(String caseId) {
		...
		given().header("Authorization", "Bearer " + jwtToken)
				...
				.when().get(url + "/cases/{caseId}/poms").then()
				 .assertThat().body(JsonSchemaValidator.matchesJsonSchemaInClasspath("MasterJsonSchema\\POM.json"))
				.log().all();
	}
